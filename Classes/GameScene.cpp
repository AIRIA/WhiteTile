...
//            tile->setTouchEnabled(false);
			...
			if (randomPos == j && row > 0)
			{
                ...
                if(row==1){
                    CCLabelTTF *startLabel = CCLabelTTF::create("开始", "Arial", 30);
                    startLabel->setColor(ccWHITE);
                    startLabel->setPosition(tile->getPosition());
                    startLabel->setTag(kStartLabel);
                    GameConfig::scroller->addChild(startLabel);
                }
			}
...
void GameScene::showGuide()
{
	...
	CCLabelTTF *guideTitle = CCLabelTTF::create(GameConfig::guideTitle.c_str(), "Arial", 40);
...
void GameScene::__showResult()
{
    ...
    CCLabelTTF *appName = CCLabelTTF::create("别踩白块儿(变态版)", "Arial", 20);
    appName->setColor(ccWHITE);
    appName->setAnchorPoint(ccp(1.0,1.0));
    appName->setPosition(VisibleRect::rightTop()-ccp(10, 10));
    resultLayer->addChild(appName);
...
void GameScene::__tileTouchUpHandler(cocos2d::CCObject *pSender)
{
	...
	if (tile->row == 1)
	{
        GameConfig::scroller->removeChildByTag(kStartLabel);
		switch (gameMode)
		{
		case kClassic:
...
void GameScene::__whiteTileTouchHandler(cocos2d::CCObject *pSender)
{
	...
	WT::Tile *tile = (WT::Tile*) pSender;
	tile->setTouchEnabled(false);
    WT::Tile *redTile = WT::Tile::create();
	redTile->setColor(ccRED);
    redTile->setScaleX(tile->getScaleX());
    redTile->setScaleY(tile->getScaleY());
    redTile->setPosition(tile->getPosition());
    tile->getParent()->addChild(redTile);
	CCActionInterval *fadeOut = CCFadeOut::create(0.2f);
	CCActionInterval *fadeIn = CCFadeOut::create(0.2f);
	CCActionInterval *fadeSeq = CCSequence::create(fadeOut, fadeIn, NULL);
	CCActionInterval *blink = CCRepeat::create(fadeSeq, 2);
	CCActionInstant *blinkHandler = CCCallFunc::create(this, callfunc_selector(GameScene::__blinkHandler));
	redTile->runAction(CCSequence::create(blink, blinkHandler, NULL));
}
